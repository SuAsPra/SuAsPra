<h1 align="center">Hi 👋, I'm Suriyan Loganathan</h1>

<h3 align="center">
🚀 AI/ML Enthusiast • Full-Stack Developer • Mobile App Developer • IoT & Embedded Systems Engineer
</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/suriyan-loganathan-b1116b320/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://github.com/SuAsPra" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://leetcode.com/u/suriyanloganathan/" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
  </a>
  <img src="https://komarev.com/ghpvc/?username=SuAsPra&label=Profile%20Views&color=0e75b6&style=for-the-badge"/>
</p>

---

# 👨‍💻 About Me

* 🔭 Currently exploring **Machine Learning, Full-Stack Development, and Embedded Systems**
* 🌱 Learning more about **Scalable Backend Systems, AI, Cloud, and Mobile Development**
* 💼 Previously worked on **startup, freelance, volunteer, and research projects**
* 🚴 Developed an AI-powered **Smart Bicycle Safety System**
* 🤝 Open to collaborating on **AI, MERN, Flutter, IoT, and Open Source Projects**
* 💡 I enjoy building products that connect **software, AI, and hardware**.

> *"I build intelligent systems that bridge AI with the physical world."*

---

# 🚀 Featured Projects

### 🚴 Smart Bicycle Safety System *(Patent Application Submitted)*

AI + Flutter + ESP32 + TensorFlow Lite + Node.js + MongoDB

* Real-time road condition detection
* GPS tracking & emergency alerts
* Health monitoring with IoT sensors
* Telegram-based SOS system

---

### 🧠 CBR Prediction using Machine Learning

* Machine learning research for predicting California Bearing Ratio (CBR)
* Feature engineering, visualization, and regression model development

---

### 📖 AI Storytelling Platform

Built during a 24-hour hackathon using Generative AI.

🌐 https://arc-night-skystory-ai-project-25dp.vercel.app/

---

### 🌐 Narrative Portfolio (MERN)

Interactive portfolio visualization built using React Flow, Express.js, and MongoDB.

---

### 🌦️ Weather Sense

Real-time weather dashboard with API integration and responsive UI.

---

# 🛠 Tech Stack

### 💻 Programming Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=java,python,c,cpp,js,html,css,sql"/>
</p>

### 🌐 Web & Mobile Development

<p align="center">
<img src="https://skillicons.dev/icons?i=react,nodejs,express,mongodb,firebase,flutter,wordpress"/>
</p>

**Also worked with:** React Native • Expo • REST APIs • Mongoose

### 🤖 AI / Machine Learning / Data Science

<p align="center">
<img src="https://skillicons.dev/icons?i=tensorflow"/>
</p>

**Libraries & Tools**

* TensorFlow Lite
* Scikit-learn
* Pandas
* NumPy
* Matplotlib
* Jupyter Notebook
* Google Colab

### 🔌 Embedded Systems & IoT

<p align="center">
<img src="https://skillicons.dev/icons?i=arduino"/>
</p>

* ESP32
* IoT Sensors
* GPS
* Electronics Prototyping

### 🛠 Tools

<p align="center">
<img src="https://skillicons.dev/icons?i=git,github,linux,vscode,figma,postman"/>
</p>

---

# 💼 Experience Snapshot

* 🔬 Machine Learning Research
* 📱 React Native Mobile Development
* 🌐 Freelance Web Development
* 🤖 AI & IoT System Development
* 🛰️ Embedded Systems
* 👥 Technical Community Management

---

# 🏆 Highlights

* 🚴 Patent Application Submitted for an AI-powered Smart Bicycle Safety System
* 🤖 Developed AI, Full-Stack, Mobile, and Embedded Systems projects
* 🌍 Built software solutions for startups, NGOs, and research
* 💻 Passionate about solving real-world engineering problems through technology

---

# 📊 GitHub Statistics

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=SuAsPra&show_icons=true&theme=tokyonight&hide_border=true" width="48%"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=SuAsPra&theme=tokyonight&hide_border=true" width="48%"/>
</p>

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SuAsPra&layout=compact&theme=tokyonight&hide_border=true" width="45%"/>
</p>

---

# 🏆 GitHub Achievements

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=SuAsPra&theme=tokyonight&no-frame=true&row=1&column=6"/>
</p>

---

# 📫 Connect With Me

📧 **[suriyanloganathan@gmail.com](mailto:suriyanloganathan@gmail.com)**

💼 **LinkedIn:**
https://linkedin.com/in/suriyan-loganathan-b1116b320

🧩 **LeetCode:**
https://leetcode.com/u/suriyanloganathan/

---

<p align="center">
⭐️ <b>Thanks for visiting my profile!</b>
</p>
